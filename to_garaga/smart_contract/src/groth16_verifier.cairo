...
trait IGroth16VerifierBN254<TContractState> {
    ...
    fn get_hash(self: @TContractState, hash: u256) -> bool;
}
...
mod Groth16VerifierBN254 {
    ...
    #[storage]
    struct Storage {
        verified_messages: Map<u256, bool>,
    }
    ...
    impl IGroth16VerifierBN254 of super::IGroth16VerifierBN254<ContractState> {
        fn verify_groth16_proof_bn254(
            ...
            if check == true {
                let hash: u256 = *groth16_proof.public_inputs[0];
                self.verified_messages.entry(hash).write(true);
                return Option::Some(groth16_proof.public_inputs);
            } else {
                return Option::None;
            }
        }
        fn get_hash(self: @ContractState, hash: u256) -> bool {
            self.verified_messages.entry(hash).read()
        }
